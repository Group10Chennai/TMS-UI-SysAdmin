...
app.controller('TMSController', ['$scope', '$rootScope', '$state', 'APIServices',
	...
	// display user vehicle details (jeni)
	...
	$scope.pageChanged_vehUserDetails = function(){
		...
		//$scope.getVehicleUserDetails($scope.nextIndex_vehUserDetails);
		$scope.getVehicleUserDetails($scope.nextIndex_vehUserDetails, $scope.searchStringForvehUserDetails);
	}
	
	
	$scope.VehicleUserDetails = new Array();
	
	$scope.getVehicleUserDetails = function(startIndex, searchWord) {
		try {
			APIServices.callGET_API($rootScope.HOST_TMS + 'api/tms/getUserVehDetials?limit='+ $scope.itemsPerPage_vehUserDetails +'&startIndex='+startIndex +'&searchWord='+searchWord, true)
			.then(
				function(httpResponse) { // Success block
					try {
						loading.finish();
						if(httpResponse.data.status == true) {
							...
							//$scope.totalItems_vehUserDetails =  20;
						}
					}
					catch(error) {
						loading.finish();
  ...
  $scope.itemsPerPage_inspection = 10;
	...
	$scope.pageChanged_sensor = function(){
 			 ...
 			 function(httpResponse){ // Success block
 				 try{
					 ...
					 if(httpResponse.data.status == true)
					 {
						...
						console.log("total sensor sizes: "+ $scope.totalItems_sensor); 
	...
	if($state.current.url == "/tms-vehicleDetails") {
	    $scope.pageChanged_vehUserDetails();
